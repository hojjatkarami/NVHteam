...
for j=1:lng
    ...
    F_h_1(:,j) = ((1i*w(j)*c_1+k_1)*[eye(3) B_1']*(-w(j)^2*M+1i*w(j)*C+K)^(-1)*Torque);
    F_h_2(:,j) = ((1i*w(j)*c_2+k_2)*[eye(3) B_2']*(-w(j)^2*M+1i*w(j)*C+K)^(-1)*Torque);
    F_h_3(:,j) = ((1i*w(j)*c_3+k_3)*[eye(3) B_3']*(-w(j)^2*M+1i*w(j)*C+K)^(-1)*Torque);
end
